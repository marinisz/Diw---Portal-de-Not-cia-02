...
function exibeNoticia() {
    ...
    for (i = 0; i < dados.articles.length; i++) {
        ...
        texto = texto + `
        <div class="div1"><img src="${noticia.urlToImage}" alt="">
                      <span class="titulo">${noticia.title}</span><br>
                      <span class="credito"> ${data.toLocaleDateString()} - ${noticia.source.name} - ${noticia.author}</span><br>
                      <span class="text">${noticia.content}<a href="${noticia.url}">Leia mais...</a> </span>
                  </div>`;
        //preencher
